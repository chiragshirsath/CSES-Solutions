/*
Question : 
input : nxm grid <=1000, with player(A), monsters(M), walls(#), floor(.)
output : can player escape to boundary? if yes print path
intution : first bfs all monsters to get their arrival time at each cell, then bfs player and only move to cells where player arrives before any monster
soln : multi-source bfs from all monsters first, then bfs from player checking arrival time, backtrack path if boundary reached
*/
#include <bits/stdc++.h>
using namespace std;
